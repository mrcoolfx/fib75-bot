...
HEADERS = {"User-Agent": "fib75-telegram-bot/1.0"}
    ...
    for chat_id, contracts in list(chat_state.items()):
        ...
        for contract, st in list(contracts.items()):
            ...
            st["pair"] = {
                ...
                "dex": pair.get("dexId"),
                "base": (pair.get("baseToken") or {}).get("symbol") or (pair.get("baseToken") or {}).get("name") or "Token",
                "quote": (pair.get("quoteToken") or {}).get("symbol") or (pair.get("quoteToken") or {}).get("name") or "",
            }
...
async def add_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 4:
        return await update.message.reply_text("Usage: /add <contract> <low_usd> <high_usd>")
    ...
    token_name = (pair.get("baseToken") or {}).get("symbol") or (pair.get("baseToken") or {}).get("name") or "Token"
...
async def list_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for contract, st in chat_state[chat_id].items():
        lo, hi = st["band"]
        lines.append(
            f"- `{contract}`  Fib75: {fmt_usd(st['fib75'])}  "
            f"Band: [{fmt_usd(lo)} — {fmt_usd(hi)}]  "
            f"Alerts: {st['alerts_sent']}/2"
        )
